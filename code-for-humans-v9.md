...
- MUST identify and preserve business invariants, transactional boundaries, data isolation rules, and non-obvious legacy constraints before changing behavior.
- MUST NOT remove unusual guards, checks, or special cases as "obsolete" until their purpose and remaining dependencies are understood.
...
- MUST NOT add fallback or compatibility behavior based only on generic examples or other APIs; require evidence from the current contract, a real response, the existing project, or an explicit migration requirement.
- Do not guess when the answer can be found in the codebase or authoritative documentation.

## 2. Trace Failures to the Root Cause

When behavior or output is missing, incorrect, inconsistent, or unexpected, trace the real execution and data flow until the root cause is identified.

- Start from the observable failure and trace backward through the actual execution or data flow.
- Identify the first point where the value, state, structure, timing, or behavior diverges from what is expected.
- Compare parallel representations of the same data when applicable, such as:
  - frontend state vs API payload
  - request DTO vs domain model
  - domain model vs persistence entity
  - database value vs returned response
  - producer state vs consumer state
  - source data vs cached or derived data
- Identify which component actually owns the incorrect state or behavior.
- Fix the owning synchronization, normalization, state transition, mapping, persistence, or contract boundary.
- MUST NOT add downstream fallbacks merely to hide an upstream defect.
- MUST NOT duplicate correction logic across consumers when the defect belongs to a shared producer or boundary.
- MUST NOT normalize the same invalid state repeatedly at multiple layers when one authoritative boundary can enforce the invariant.
- MUST NOT treat the final visible symptom as the root cause without tracing how it was produced.
- Prefer correcting the earliest authoritative divergence that can safely restore the intended invariant.
- Preserve valid downstream assumptions instead of weakening them to tolerate incorrect upstream state.
- Verify the fix with the narrowest relevant check.
- Review the final diff for defensive patches, duplicate normalization, hidden fallbacks, and unrelated changes.

## 3. Prefer Existing Code Before Adding More
...
## 4. Design Only for Known Variation

Design for current and explicitly confirmed variation, not hypothetical future requirements.

- For known variation, design the smallest stable extension point that the current requirements justify.
- For probable but unconfirmed variation, keep implementation details localized and boundaries clean, but MUST NOT introduce new architectural abstractions, extension mechanisms, or generic frameworks solely for that possibility.
- For hypothetical variation without concrete evidence, do not design for it.
...
Before claiming completion, MUST verify every applicable item below:

- [ ] The requested behavior is complete and within scope.
- [ ] The actual execution and data flow were understood before modification.
- [ ] The root cause was identified instead of only treating the visible symptom.
- [ ] Missing, incorrect, or inconsistent behavior was traced through the real flow where applicable.
- [ ] The earliest meaningful divergence was identified where applicable.
- [ ] The owning boundary was fixed instead of adding downstream fallback patches.
- [ ] Existing code was reused, modified, simplified, or deleted before unnecessary code was added.
- [ ] No speculative architecture, duplicate owner, parallel path, dead code, or accidental edit remains.
- [ ] Known variation uses only the smallest justified extension point.
- [ ] Probable but unconfirmed variation affects boundaries only, not architecture.
- [ ] The business flow remains cohesive and easy to follow.
- [ ] Every touched file and every new method, class, or abstraction has a concrete reason to exist.
- [ ] No obsolete implementation remains after a completed replacement.
- [ ] Relevant verification passed, or anything that could not be verified is explicitly reported.
